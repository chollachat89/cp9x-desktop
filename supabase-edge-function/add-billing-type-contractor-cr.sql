...
--
-- ค่าที่ใช้ได้หลังรันไฟล์นี้มี 3 แบบ (ชื่อที่แสดงในแอปตั้งแต่ v1.0.38):
--   'normal'        = เก็บเงินปกติ        -> ออกใบวางบิลทั้งฝั่ง CJ และฝั่งผู้รับเหมา (ค่าเริ่มต้น)
--   'claim'         = เคลมประกัน 3 เดือน  -> ไม่เก็บเงินทั้ง 2 ฝั่ง (ไม่ขึ้นทั้งบิล CJ และบิลผู้รับเหมา)
--   'contractor_cr' = เคลมอะไหล่          -> ขึ้นเฉพาะใบวางบิลฝั่งผู้รับเหมา ฝั่ง CJ ไม่เก็บ
--
-- ค่าที่เก็บจริงในฐานข้อมูลยังเป็น 'claim' / 'contractor_cr' เหมือนเดิม เปลี่ยนแค่ชื่อที่แสดงให้ผู้ใช้เห็น
-- จึงไม่ต้องแก้ข้อมูลเก่าใด ๆ ตอนเปลี่ยนชื่อ
--
